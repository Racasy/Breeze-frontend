    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center sm:ml-6">
                ...
                <Dropdown>
                    ...
                    <template #content>
                        <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" v-for="company in companies" :key="company.id" @click.prevent="selectCompany(company)">
                            {{ company.company_name }}
                        </a>
                        <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" @click.prevent="selectCompany(null)">
                            All Companies
                        </a>
                    </template>
                </Dropdown>
        ...
        <div class="py-12 flex justify-center">
            <div class="bg-white shadow-sm rounded-md p-4">
                <AdminFileTable :selectedCompany="selectedCompany" />
            </div>
        </div>
    </AuthenticatedLayout>
</template>
